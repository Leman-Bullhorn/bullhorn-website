    ...
    () =>
      (articleData?.pages ?? [])
        ...
        .map(it => ({
          ...
          published: it.publicationDate.toLocaleDateString(),
          author: `${it.writer.firstName} ${it.writer.lastName}`,
        })),
    [articleData],
  );
      ...
      <Container>
        <Row xs={2}>
          <Col
            ...
            <InfiniteScroll
              dataLength={
                articleData?.pages.reduce(
                  (acc, it) => acc + it.content.length,
                  0,
                ) ?? 0
              }
